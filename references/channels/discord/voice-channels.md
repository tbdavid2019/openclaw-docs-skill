...
- DAVE's native encryption pool initializes one Rayon thread per CPU on the first membership rekey and is outside JavaScript Worker counts. Operators can bound it by setting `RAYON_NUM_THREADS` in the Gateway service environment; OpenClaw does not set this variable. The limit also applies to child processes that inherit that environment.
