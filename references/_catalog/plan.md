...
- [Runners plan](../plan/runners.md) — Everything is a node — one placement model where paired machines and cloud boxes run OpenClaw worker turns or Codex remote execution through mode-specific admission; clients attach to sessions, never to runners. Read when: Designing or reviewing where sessions run (gateway, device, cloud); Changing the Where picker, device pairing, node onboarding, or worker dispatch surfaces; Naming anything around sessions, devices, nodes, or placement.
