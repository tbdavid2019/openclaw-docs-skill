...
- [Nodes](../nodes/index.md) — Nodes: pairing, capabilities, permissions, and CLI helpers for camera/screen/device/notifications/system and the macOS widget panel. Read when: Pairing iOS/watchOS/Android nodes to a gateway; Using node camera or screen capture for agent context; Presenting a hosted widget on a Mac; Adding new node commands or CLI helpers.
